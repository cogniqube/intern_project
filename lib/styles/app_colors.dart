import 'package:flutter/material.dart';
import 'dart:ui';

class AppColors{
  static const primary = Color(0xFFE66100);
  static const font = Color(0xFF303030);
  static const font2 = Color(0xFF373737);
  static const disablefont = Color(0xFF707070);
  static const unselectedButton = Color(0xFF303030);
  static const background = Color(0xfffdfffe);
  static const white = Color(0xffffffff);
  static const black = Color(0xff000000);
  static const appbar = Color(0xfff5f1f1);
  static const tabBar = Color(0xFFF5F5F5);
  static const sponsor = Color(0xFFF2B705);
  static const orange=Color(0xFFE63100);
  static const purple=Color(0xFF693ACC);
  static const grey=Color(0xff979797);
  static const red=Color(0xffE64646);
  static const blue=Color(0xff3F8AE0);
  static const lightorange=Color(0x4DE66100);

}